...
import type { AvanceTerreno, PlanSemanal } from "@prisma/client";
...
type PlanSemanalConEstado = PlanSemanal & {
  estadoCalculado: EstadoCalculado;
};

function calcularEstadoPlanSemanal(plan: PlanSemanal): EstadoCalculado {
...
function obtenerCumplimientoPorAvance(planes: PlanSemanal[]) {
...
function obtenerUltimasSemanas(planes: PlanSemanalConEstado[]) {
  ...
  return semanasUnicas.map((semana, index) => {
    ...
    const cumplimiento = calcularPorcentaje(
      avanceRealSemana,
      avanceProgramadoSemana
    );

    const completadas = planesSemana.filter(
      (plan) => plan.estadoCalculado === "COMPLETADA"
    ).length;

    return {
      semana:
        index === semanasUnicas.length - 1
          ? "Actual"
...
export async function getDashboardResumen() {
  const [planes, avancesTerreno, totalActividadesMaestras, lookaheads] =
    await Promise.all([
      prisma.planSemanal.findMany({
        orderBy: { semana: "asc" },
      }),
      prisma.avanceTerreno.findMany(),
      prisma.actividad.count({
        where: { activo: true },
      }),
      prisma.lookahead.findMany(),
    ]);

  const planesConEstado: PlanSemanalConEstado[] = planes.map(
    (plan: PlanSemanal) => ({
      ...plan,
  ...
  const completadasPlanSemanal = planesConEstado.filter(
    (plan) => plan.estadoCalculado === "COMPLETADA"
  ).length;

  const enProcesoPlanSemanal = planesConEstado.filter(
    (plan) => plan.estadoCalculado === "EN_PROCESO"
  ).length;

  const atrasadasPlanSemanal = planesConEstado.filter(
    (plan) => plan.estadoCalculado === "ATRASADA"
  ).length;

  const noIniciadasPlanSemanal = planesConEstado.filter(
    (plan) => plan.estadoCalculado === "NO_INICIADA"
  ).length;

  const avancePlanSemanal = obtenerCumplimientoPorAvance(planesConEstado);

  const completadasTerreno = avancesTerreno.filter(
    (avance: AvanceTerreno) => avance.estado === "COMPLETADA"
  ).length;

  const enProcesoTerreno = avancesTerreno.filter(
    (avance: AvanceTerreno) => avance.estado === "EN_PROCESO"
  ).length;

  const atrasadasTerreno = avancesTerreno.filter(
    (avance: AvanceTerreno) => avance.estado === "ATRASADA"
  ).length;

  const noIniciadasTerreno = Math.max(
  ...
  const liberadasLookahead = lookaheads.filter(
    (item) => item.estadoLiberacion === "LIBERADA"
  ).length;

  const noLiberadasLookahead = lookaheads.filter(
    (item) => item.estadoLiberacion === "NO_LIBERADA"
  ).length;

  const programadasLookahead = lookaheads.filter(
    (item) => item.estadoLiberacion === "PROGRAMADA"
  ).length;

  const enGestionLookahead = lookaheads.filter(
    (item) => item.estadoRestricciones === "EN_GESTION"
  ).length;

  const vencidasLookahead = lookaheads.filter(
    (item) => item.estadoRestricciones === "VENCIDA"
  ).length;
  ...
  return {
    avanceTerreno: {
      ...
      avanceGeneralObra: calcularPorcentaje(
        completadasTerreno,
        totalActividadesMaestras
      ),
    },
    ...
    lookahead: {
      ...
      porcentajeLiberadas: calcularPorcentaje(
        liberadasLookahead,
        totalLookahead
      ),
      porcentajeProgramadas: calcularPorcentaje(
        programadasLookahead,
        totalLookahead
      ),
      porcentajeEnGestion: calcularPorcentaje(
        enGestionLookahead,
        totalLookahead
      ),
      porcentajeVencidas: calcularPorcentaje(
        vencidasLookahead,
        totalLookahead
      ),
    },
